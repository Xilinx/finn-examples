...
class DetectorDriver():
    ...
    def __init__(self, bitfile_name, platform, io_shape_dict, quant_tail_params_dir, batch_size=1, runtime_weight_dir="runtime_weights/", device=None):
        ...
        self.muls = [np.load(join(quant_tail_params_dir, "Mul_{}_param0".format(i))) for i in range(io_shape_dict['num_outputs'])]
        self.adds = [np.load(join(quant_tail_params_dir, "Add_{}_param0".format(i))) for i in range(io_shape_dict['num_outputs'])]
