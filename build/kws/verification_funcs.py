...
def verify_build_output(cfg, model_name):
    ...
    if (os.getenv("VERIFICATION_EN")) == "1":
        ...
        if os.path.isdir(verif_output_dir) == False:
            ...
            return
        logger.info("\n*****************************************************")
        logger.info("Verification Results for %-10s on %-10s" % (model_name, cfg.board))
        logger.info("*****************************************************")
    ...
    if "qonnx_to_finn_python" in cfg.verify_steps:
        cfg.verify_steps = [step.replace("qonnx_to_finn_python", "finn_onnx_python") for step in cfg.verify_steps]
